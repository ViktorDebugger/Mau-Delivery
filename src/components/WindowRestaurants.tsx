...
const WindowRestaurants = ({ handle }: SearchInputProps) => {
        ...
        <ul className="flex justify-center flex-wrap mt-[30px] gap-[30px]">
          {[1, 2, 3, 4, 5, 6].map((item) => (
            <li key={item} className="flex-1/3 flex items-center gap-3 rounded-4xl">
              <Image
                src="/images/icons/mcdonalds.png"
                alt="restaurant"
                width={100}
                height={100}
              />
              <div className="font-katibeh">
                ...
                <h2 className="text-[25px] leading-[2.3]">Street</h2>
                <div className="flex justify-end">
                <ul className="bg-white px-2 rounded-4xl flex w-[70%] gap-1">
                    {[1, 2, 3, 4, 5].map(item => (
                        <li key={item} className="text-yellow-300 text-2xl">
                            ★
                        </li>
                    ))}
                </ul>
...
export default WindowRestaurants;
